...
$("#subscribe").submit(function (e) {
    ...
    var mail = oForm.elements["email"];

    if (isValidEmail(email)) {
        $.ajax({
            type: "POST",
            url: "http://backend-51053.onmodulus.net/api/v1/subscribe",
            data: dataString,
            success: function () {
                $('.subscription-success').fadeIn(1000);
                $('.subscription-error').fadeOut(500);
                $('.hide-after').fadeOut(4000);
                mail.value = "";
...
$("#contact").submit(function (e) {
    ...
    var oForm = document.forms["message-form"];
    var nom = oForm.elements["name"];

    if (isValidEmail(email) && (message.length > 1) && (name.length > 1)) {
        $.ajax({
            type: "POST",
            url: " http://backend-51053.onmodulus.net/api/v1/quotes",
            data: dataString,
            success: function () {
                $('.success').fadeIn(1000).delay(3000).fadeOut(4000);
                $('.error').fadeOut(500);
